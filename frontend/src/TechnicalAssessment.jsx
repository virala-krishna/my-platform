...
export default function TechnicalAssessment({ onComplete, userSelections, onClose }) {
  ...
  const [timeLeft, setTimeLeft] = useState(600); // 10 minutes for quick assessment
  const [selectedOption, setSelectedOption] = useState(null);
  const [showExplanation, setShowExplanation] = useState(false);

  useEffect(() => {
    // Load questions based on user selections
    const assessmentQuestions = getQuestionsForAssessment(userSelections, 10); // Reduced to 10 questions for quick assessment
  ...
  const restartAssessment = () => {
    ...
    const newQuestions = getQuestionsForAssessment(userSelections, 10);
  ...
  return (
    <div className="wrap">
      ...
      <div className="panel">
        ...
        {showExplanation && (
          <div style={{ 
            ...
            <h4 style={{ color: selectedOption === currentQ.correct ? 'var(--ok)' : 'var(--danger)', marginBottom: '10px' }}>
      ...
      <div style={{ 
        ...
        <div className="panel" style={{ textAlign: 'center', padding: '15px' }}>
          <h3 style={{ margin: '0', color: 'var(--muted)' }}>{currentQuestion - score}</h3>
          <p style={{ margin: '5px 0 0', color: 'var(--muted)' }}>Incorrect</p>
        </div>
        <div className="panel" style={{ textAlign: 'center', padding: '15px' }}>
          <h3 style={{ margin: '0', color: 'var(--ok)' }}>{Math.round((score / (currentQuestion + 1)) * 100)}%</h3>
